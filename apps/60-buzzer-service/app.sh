...
start() {
    ...
    socat TCP-LISTEN:7777,fork,reuseaddr EXEC:"/bin/sh ${APP_ROOT}/buzzer_wrapper.sh" >> "$LOG" 2>&1 &
...
stop() {
    ...
    # Ensure buzzer is turned off when stopping
    if [ -d "/sys/devices/platform/ff350000.pwm/pwm/pwmchip0/pwm0" ]; then
        echo 0 > /sys/devices/platform/ff350000.pwm/pwm/pwmchip0/pwm0/enable || true
        echo 0 > /sys/devices/platform/ff350000.pwm/pwm/pwmchip0/pwm0/duty_cycle || true
    fi
